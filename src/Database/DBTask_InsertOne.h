...
public:
    ...
    TDBTask_InsertOne(std::string collection, Callback &&callback, bsoncxx::document::value document, mongocxx::options::insert options = {})
        : TDBTaskBase<Callback>(std::move(collection), std::forward<Callback>(callback)),
          mDocument(std::move(document)), mOptions(std::move(options)) {
    }
